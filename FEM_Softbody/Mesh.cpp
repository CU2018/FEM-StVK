...
bool TetMesh::init(std::string fileName, ScalarType unitMass)
{
...
bool TetMesh::exportToFile(int frameNum)
{
	...
	std::string fileName = "tetOutput/tet." + std::to_string(frameNum) + ".tet";
...
void TetMesh::generateParticleList()
{
	...
	assert(unitMass - 0.0f != EPSILON);
	totalMass = unitMass * systemDimension;
